// fetch를 통해서 되돌려 받은 주문 리스트를 왼쪽으 주문리스트에 추가하기
const add_order_list = (order_list) => {
  ...
  // div.order_list 를 가져와서, 전체를 article.order_list로부터
  // 삭제하기
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // butto 클리되었을때 결제 처리를 수행하려고 했다
  // button box 포함하여 button 을 동적으로 생성을 했다
  // 동적으로 생성된 tag들은 자체적으로 event를 수신하지 못한다
  // 아래의 event 핸들러는 div.button_box 가 만들어 지기 전에
  // 선언되고 OS에게 알려진 코드이다
  ...
  // 처음에 아예 전체 HTML 문저자체에 click event를 설정해 둔다
  ...
  document
    ...
    .addEventListener("click", (e) => {
      if (confirm("결제를 진행할까요")) {
        ...
        fetch(`/pos/paycomplate/${table_id}`)
          .then((res) => res.text())
          .then((result) => {
            if (result === "OK") {
              document.querySelector("div.modal").style.display = "none";
